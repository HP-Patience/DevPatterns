...
**同步更新阅读状态**：在 `信息汇报/阅读状态.md` 末尾追加一条未读条目：
```markdown
- [ ] YYYY-MM-DD-主题.md
```
...
- 打开 `信息汇报/阅读状态.md`
- 看完某篇汇报后，将其对应条目的 `[ ]` 改为 `[x]`，并追加完成日期
  ```markdown
  - [x] YYYY-MM-DD-主题.md `2026-06-25`
  ```

### ⑥ 归档 — 触发时机

用户单独发指令（如"归档已读汇报"），AI 执行以下操作：
...
**汇报归档：**
- 每条已读汇报文件 frontmatter 追加 `read: {{完成日期 YYYY-MM-DD}}`
- 从 `信息汇报/` 移入 `Archive/信息汇报/{{归档年月}}/`
- 在 `阅读状态.md` 中对应条目标记为已完成：
  ```markdown
  - [x] ~~YYYY-MM-DD-主题.md~~ `2026-06-25` ✅ 已归档
  ```

**信息源归档：**
- 已汇报的信息源笔记从 `信息源/` 移入 `Archive/信息源/{{归档年月}}/`
- Inbox 中对应 URL 标记 `✅`

> 原则：信息源是"正在处理"，Archive 是"已完结"。阅读状态保留完整记录。按月份归档，方便回溯。
...
- `阅读状态.md` 是唯一阅读记录，用户在此勾选已读
- AI 不可替用户勾选，只负责追加新条目和归档时标记已完成
- 已归档条目保留在文件中，不删除，保持完整阅读历史
...
**4. 更新阅读状态** — 在 `信息汇报/阅读状态.md` 末尾追加：
- [ ] YYYY-MM-DD-主题.md

**5. 清理** — 选中笔记从 `信息源/` 移入 `Archive/信息源/{{当前年月}}/`。Inbox 中对应 URL 标记 `✅`。

**6. 输出** — 将汇报内容写入 `信息汇报/` 目录，文件名格式 `YYYY-MM-DD-主题.md`。
```
...
**1. 检查阅读状态** — 读取 `信息汇报/阅读状态.md`，找出所有 [x] 条目（已读）。

**2. 归档汇报** — 对每条已读汇报文件：
  - frontmatter 追加 read: {{完成日期}}
  - 从 `信息汇报/` 移入 `Archive/信息汇报/{{当前年月}}/`
  - 在 阅读状态.md 中对应条目标记：
    - [x] ~~YYYY-MM-DD-主题.md~~ `完成日期` ✅ 已归档

**3. 归档信息源** — 信息源/ 中的 reported 笔记移入 Archive/信息源/{{当前年月}}/。Inbox 中对应 URL 标记 ✅。

执行前用 AskUserQuestion 确认待归档列表。
```
