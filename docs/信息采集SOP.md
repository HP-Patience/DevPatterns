...
**0. 确认范围** — 使用 AskUserQuestion 工具询问用户要处理哪些笔记。列出 `信息源/` 目录下的所有文件，让用户选择。用户确认后只处理选中的文件。

**1. 汇报** — 阅读选中的笔记，输出一份信息汇报：
  - 每条信息核心观点（附 [[笔记名]] 来源链接）
  - 跨信息共性模式或矛盾点
  - 对我有价值的行动建议
  - 哪些值得深入跟进

**2. 补元数据** — 每条选中笔记检查 frontmatter：
  - `tag` 为空则按内容主题补标签（从标签体系选，数组格式）
  - `description` 为空则补一句话摘要
  - `created` 缺失则填当天日期
  - 统一为 Type A 格式（title / source / created / description / tag / status）

**3. 标记** — 选中笔记将其 frontmatter 中 `status` 字段改为 `reported`。

**4. 清理** — 选中笔记从 `信息源/` 移入 `Archive/{{当前年月}}/`。Inbox 中对应 URL 标记 `✅`。
