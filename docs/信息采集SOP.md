...
**2. 标记** — 汇报中用到的每条信息源笔记，将其 frontmatter 中 `status` 字段改为 `reported`。

**3. 清理** — 汇报中用到的信息源笔记，从 `信息源/` 移入 `Archive/{{当前年月}}/`。Inbox 中对应 URL 标记 `✅`。

**4. 输出** — 将汇报内容写入 `信息汇报/` 目录，文件名格式 `YYYY-MM-DD-主题.md`。
```
